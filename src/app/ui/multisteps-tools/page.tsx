...
import type { ChatMessage } from "@/app/api/multistep-tools/route";

export default function MultiStepToolsPage() { 
     
    const { messages, sendMessage, status, error, stop } = useChat<ChatMessage>({
        transport: new DefaultChatTransport({
            api: '/api/multistep-tools',
        }),
    });
                                    ...
                                    case "tool-getLocation": {
                                        switch (part.state) { 
                                            ...
                                            case 'input-available':
                                                return (<div key={`${message.id}-getWeather-${index}`}>
                                                    <p>Getting weather for { part.input.name} user</p>
                                                </div>);
                                            case 'output-available':
                                                return (<div key={`${message.id}-getWeather-${index}`}>
                                                        Location is {part.output}
                                                </div>)
                                            case 'output-error':
                                                return (<div key={`${message.id}-getWeather-${index}`}>
                                                    Error during getLocation tool output: {part.errorText}
                                                </div>)
                                            default:
                                                return <p>Unknonw part type for tool getLocation {JSON.stringify(part, null, 2)} </p>
                                        }
                                    }
